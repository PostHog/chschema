...
ClickHouse Cloud rewrites MergeTree-family DDL as it is created. Such tables
are intercepted and created with a `Shared<X>MergeTree` engine, and reported
that way in `system.tables`:
...
`hclexp` queries the server for the setting that controls this behaviour,
`cloud_mode_engine`. When this is in a rewriting mode, `hclexp` reads
`Shared<X>MergeTree` as the plain `<X>MergeTree` and drops the autogenerated replication engine args.
From the user's perspective, declare `merge_tree` or
`replacing_merge_tree` as you would anywhere else, and the same HCL works
against Cloud and a self-hosted single node.

The capability probe is best effort. If the connection cannot read
`system.settings`, `hclexp` warns and leaves `Shared*` engines strict rather
than blocking otherwise-valid self-hosted introspection or guessing that a
plain engine will round-trip. On Cloud, grant the introspection user `SELECT`
on `system.settings`; without it, a `Shared*` table fails with an unsupported
engine error.
...
That emits `ENGINE = ReplacingMergeTree(version)`, which Cloud rewrites back
to `SharedReplacingMergeTree` on apply. The next introspect reads it as
`replacing_merge_tree` again, so the round trip reports no drift.

Two cases still fail rather than guess. A `Shared*` engine whose argument pair
is not the one Cloud generates is an error, not a silent drop, since a dropped
argument round-trips as a false "no drift". And `sql2hcl` reads DDL from a file
with no server to ask, so it rejects Cloud DDL outright.

**Declare the plain family for Cloud.** A table declared
`replicated_merge_tree` emits `ReplicatedMergeTree('/your/path', …)`; Cloud
rewrites that to `SharedMergeTree` with its own path, the next introspect reads
it as `merge_tree`, and the diff reports a permanent engine change. Declare
`merge_tree` instead. Where one schema targets both Cloud and a replicated
self-hosted cluster, put the engine in a per-environment layer and `patch_table`
it, as with any other environment difference.
